<#
.SYNOPSIS
  Instala Bridge Código de Barras en Windows / Windows Server.
...
╔══════════════════════════════════════════════════════╗
║   Bridge Código de Barras — Instalador (Windows)    ║
║   Express → SQL Server                              ║
╚══════════════════════════════════════════════════════╝
...
if (-not (Get-Command node -ErrorAction SilentlyContinue)) {
  Write-Step "Node.js no está instalado. Instalando..."
...
if (-not (Get-Command npm -ErrorAction SilentlyContinue)) {
  Write-Err "npm no está disponible después de instalar Node.js."
}
Write-Ok "Node.js $(node -v) — npm $(npm -v)"

# ── 2. Git (para clonar) ────────────────────────────────────
if (-not (Get-Command git -ErrorAction SilentlyContinue)) {
  Write-Step "Git no está instalado. Instalando..."
...
╔══════════════════════════════════════════════════════╗
║   ✅ Bridge instalado y ejecutándose                 ║
╚══════════════════════════════════════════════════════╝
